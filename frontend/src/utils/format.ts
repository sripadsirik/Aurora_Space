...
/**
 * Formats a solar-wind–magnetosphere coupling electric field in mV/m for
 * display. Renders two fraction digits with a `mV/m` suffix (for example
 * `5.58 mV/m`). Negative or non-finite inputs render as an em dash so a bad feed
 * value never shows as `NaN mV/m`.
 */
export const formatElectricFieldMvM = (fieldMvM: number): string => {
