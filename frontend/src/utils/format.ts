...
/**
 * Formats a dawn-dusk merging electric field given in millivolts per metre for
 * display. Renders two fraction digits with a `mV/m` suffix (for example
 * `2.00 mV/m`). Negative or non-finite inputs render as an em dash so a bad
 * feed value never shows as `NaN mV/m`.
 */
export const formatMergingField = (fieldMvM: number): string => {
