...
func TestClassifyOrbitPR45(t *testing.T) {
	tests := []struct {
		name       string
		altitudeKm float64
		want       string
...
func TestClassifyRiskPR45(t *testing.T) {
	tests := []struct {
		name        string
		probability float64
		want        string
...
func TestDeriveStormLevelPR45(t *testing.T) {
	tests := []struct {
		name string
		kp   float64
		want string
		...
		{"quiet", 0, "none"},
		{"unsettled below minor", 4.9, "none"},
		{"minor threshold", 5, "minor"},
		{"minor band", 5.6, "minor"},
		{"moderate threshold", 6, "moderate"},
		{"strong threshold", 7, "strong"},
		{"severe threshold", 8, "severe"},
		{"extreme threshold", 9, "extreme"},
		{"above scale", 9.5, "extreme"},
	}
