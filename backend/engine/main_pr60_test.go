...
func validRecordPR60() gpRecord {
	return gpRecord{
		NoradCatID:   25544,
		TLELine1:     issLine1PR60,
		TLELine2:     issLine2PR60,
		MeanMotion:   15.72125391,
		Eccentricity: 0.0006703,
	}
}

func TestIsTLEValidPR60(t *testing.T) {
	t.Run("accepts a well-formed record", func(t *testing.T) {
		if !isTLEValid(validRecordPR60()) {
	...
	cases := []struct {
		...
		{"line1 wrong prefix", func(r *gpRecord) { r.TLELine1 = "9" + issLine1PR60[1:] }},
		{"line2 wrong prefix", func(r *gpRecord) { r.TLELine2 = "9" + issLine2PR60[1:] }},
...
func TestTLECacheSetAndSnapshotPR60(t *testing.T) {
	cache := &tleCache{records: make(map[string]gpRecord)}

	cache.set("25544", validRecordPR60())
