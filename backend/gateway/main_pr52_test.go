...
func TestPayloadCountPR52(t *testing.T) {
	cases := []struct {
		name    string
		msgType string
		payload string
...
func decodeSatelliteIDsPR52(t *testing.T, payload json.RawMessage) []int {
	t.Helper()
...
func TestSatelliteBatchAssemblerPassThroughPR52(t *testing.T) {
	...
	t.Run("a bare satellite array passes straight through", func(t *testing.T) {
		...
		if ids := decodeSatelliteIDsPR52(t, payload); len(ids) != 2 {
	...
	t.Run("a single-batch envelope completes immediately", func(t *testing.T) {
		...
		if ids := decodeSatelliteIDsPR52(t, payload); len(ids) != 1 || ids[0] != 7 {
...
func TestSatelliteBatchAssemblerReassemblyPR52(t *testing.T) {
	a := newSatelliteBatchAssembler()
	...
	if ids := decodeSatelliteIDsPR52(t, payload); len(ids) != 2 || ids[0] != 10 || ids[1] != 20 {
